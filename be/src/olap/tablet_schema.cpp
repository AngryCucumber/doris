...
const TabletIndex* TabletSchema::geo_index(int32_t col_unique_id,
                                           const std::string& suffix_path) const {
    for (size_t i = 0; i < _indexes.size(); i++) {
        if (_indexes[i]->index_type() == IndexType::GEO) {
            for (int32_t id : _indexes[i]->col_unique_ids()) {
                if (id == col_unique_id &&
                    _indexes[i]->get_index_suffix() == escape_for_path_name(suffix_path)) {
                    return _indexes[i].get();
...
const TabletIndex* TabletSchema::geo_index(const TabletColumn& col) const {
    if (!segment_v2::IndexColumnWriter::check_support_geo_index(col)) {
        return nullptr;
    }
    int32_t col_unique_id = col.is_extracted_column() ? col.parent_unique_id() : col.unique_id();
    return geo_index(col_unique_id, escape_for_path_name(col.suffix_path()));
}
